...
const { Client } = require('dsocialjs');
...
const dpid = dpayid.Initialize({ app: 'dsocial.dapp' });
...
const dpaydWsUrl = process.env.DPAYD_WS_URL || 'wss://greatchain.dpays.io';
const client = new Client(dpaydWsUrl);
...
wss.on('connection', ws => {
  ...
  ws.on('message', message => {
    ...
    if (call.method === 'get_notifications' && call.params && call.params[0]) {
      ...
      dpid.setAccessToken(call.params[0]);
      dpid
        .me()
        .then(result => {
      ...
      case 'custom_json': {
        ...
        switch (params.id) {
          case 'follow': {
            ...
            /** Find repost */
            if (json[0] === 'repost' && json[1].account && json[1].author && json[1].permlink) {
              const notification = {
                type: 'repost',
              ...
              // console.log('Repost', [json[1].author, JSON.stringify(notification)]);
